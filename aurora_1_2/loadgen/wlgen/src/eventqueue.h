...
//
// Insert an event "p_func" with label "p_string" to fire at 
// time "p_time" with data "p_data".  "p_func" is of type
// typedef void (*eventfunc_t) (void*); if p_relative_time = 1, use
// relative time in p_time instead of real time.
// :
//    m_eq.AddEvent(p_string, p_time, p_func, p_data, p_relative_time);
//                                 // insert an event with label
//                                 p_string
//
// For example:
//    m_eq.AddEvent(
...
// A shorthand typedef for the thread functions.
typedef void* (*Threader) (void*);

// A shorthand typedef for EventQueue functions
typedef void (*eventfunc_t) (void*);

// generates a timegen with p_secs plus 0..p_rand seconds
// yes, it should use a seed...
...
//        std::cerr << "seconds=" << item.time.tv_sec
//                  << " usecs="  << item.time.tv_usec
//                  << std::endl;
