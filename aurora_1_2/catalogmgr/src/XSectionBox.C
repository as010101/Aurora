...
XSectionBox::XSectionBox(int id, float cost, float sel, string modifier, int x, int y, int width, int height):
  Box(id, cost, sel, x, y, width, height)
{
  ...
  while ((j < len) && (temp[j] != '='))
    j++;
  if (j == len) {
    printf("No '=' character found in XSection expression. Giving up...\n");
    abort();
  }
  ...
  i = 0;
  temp = (v[4].c_str());
  while ((i < v[4].size()) && (!isalpha(temp[i])))
    i++;
  if (i == v[4].size()) {
  ...
  if (strncmp(temp+i, "TIMEOUT", 7) == 0) {
   ...
   while (temp[i] != ' ')
     i++;
   char *pred_string = new char[v[4].size() - i - 1];
    int k = 0; 
    while ((i + k) <= v[4].size())
      {
	pred_string[k] = temp[i + k];
  ...
  //than a time xsection parameter, the following line of code will be deleted.
...
XSectionBox::~XSectionBox()
{
}

AggregateState *XSectionBox::getState()
{
  return m_state;
}

int XSectionBox::getBoxType()
{
  return XSECTION;
}

string XSectionBox::toString()
{
  strstream s;
  s << "XSectionBox (" << Box::toString() << ", expression: " << m_modifier << ")" << '\0';
  return s.str();
}
