...
TumbleBox::TumbleBox(int id, float cost, float sel, string modifier, int x, int y, int width, int height):
  Box(id, cost, sel, x, y, width, height)
{
  ...
  while ((j < len) && (temp[j] != '='))
    j++;
  if (j == len) {
    printf("No '=' character found in Tumble expression. Giving up...\n");
    abort();
  }
  ...
  else if (strcmp(to_parse, "LASTVAL") == 0)
    {
      if (temp_type == 'i')
	m_state->af = new IntLastValueAF(v[1].c_str());
      else
  	m_state->af = new FloatLastValueAF(v[1].c_str());
    }
  else
    {
      printf("Aggegate function not {AVG, COUNT, SUM, MAX, MIN, LASTVAL}. Giving up...\n");
      abort();
    }
  delete[] to_parse;
  ...
  //than a time tumble parameter, the following line of code will be deleted.
  //m_state->af = new IntSumAF(":0:i:0:4:");
  //m_state->output = ALL;
  //m_state->until = NONE;
  //m_state->until = SATISFIES;
  //m_state->satisfies_pred = p->parsePred(":0:i:0:4: > 5");
  //m_state->slack_time = 0;
  //m_state->group_by = " :0:i:9:4:";
  //  m_state->unique_time = 10; //HACK...see above
  m_state->unique_time = *(new Timestamp(10, 0));  // HACK .... see above

  m_state->reg_hash = new Hash();
  m_state->last = new Hash();
...
AggregateState *TumbleBox::getState()
{
  return m_state;
}

int TumbleBox::getBoxType()
{
  return TUMBLE;
}

string TumbleBox::toString()
{
  strstream s;
  s << "TumbleBox (" << Box::toString() << ", expression: " << m_modifier << ")" << '\0';
  return s.str();
}
